...
class ExportTerminalsCommand extends BaseCommand {
  async execute({ title = 'POS Terminals Export', summary = 'Live export of all POS terminals' } = {}) {
    ...
    try {
      ...
      await this.navigationRouter.navigate(page, 'pos-terminals');
      ...
      const snapshotPath = this.snapshotManager.save({ route: 'pos-terminals', title, summary, rows, generatedAt: new Date().toISOString() });
      ...
      return { route: 'pos-terminals', excelPath, wordPath, snapshotPath, rows };
    } catch (error) {
